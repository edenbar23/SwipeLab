...
export default function StepConfirm({ formData, onBack, onSubmit, loading, availableOptions }: StepConfirmProps) {
  ...
  return (
    <View style={[styles.container, isWeb && styles.containerWeb]}>
      <Text style={[styles.heading, { color: themeColors.text }, isWeb && styles.headingWeb]}>Review & Confirm</Text>
              ...
              {recipientLabels.length > 0 ? (
                <View style={styles.tagsRow}>
                  {recipientLabels.map((label, i) => (
                    <View key={i} style={[styles.tag, styles.recipientTag]}><Text style={styles.recipientTagText}>{label}</Text></View>
                  ))}
                </View>
              ) : (
      ...
      <View style={[styles.footer, isWeb && styles.footerWeb]}>
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity style={[styles.submitButton, loading && styles.buttonDisabled]} onPress={onSubmit} disabled={loading}>
...
const styles = StyleSheet.create({
  ...
  tag: { backgroundColor: '#10B981', paddingVertical: 6, paddingHorizontal: 14, borderRadius: 20 },
  tagText: { color: '#fff', fontWeight: '600', fontSize: 13 },
  recipientTag: { backgroundColor: 'rgba(16, 185, 129, 0.12)' },
