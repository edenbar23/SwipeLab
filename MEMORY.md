...
* Security/Auth: Optimized StarDBI Authentication by implementing a BFF pattern and eliminating `ExternalAuthFilter`. StarDBI tokens are now securely cached backend-side via Caffeine (`STARDBI_TOKENS_CACHE`), completely hiding them from the frontend. Implemented `StardbiAuthService.executeWithStardbiToken` wrapper to seamlessly handle Stardbi 401s, token rotation, and graceful fallback to a service account for background synchronization. React Native frontend was updated to strip all explicit StarDBI token handling (`X-Stardbi` headers) and relies purely on native SwipeLab JWTs.
