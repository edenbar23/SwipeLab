...
// Classification.builder().user(testUser).image(testImage).label(testLabel).build();
...
// when(credibilityCalculator.calculateCohenKappa(anyList(),
// anyList())).thenReturn(0.85);

// // Mock Majority Agreement Logic
// // For majority agreement loop
// when(classificationRepository.findByImageId(imageId)).thenReturn(List.of(userClass,
// userClass)); // Need >1 for
// // majority
// when(credibilityCalculator.calculateMajorityVote(anyList())).thenReturn(testLabel);
// when(credibilityCalculator.calculateMajorityAgreementScore(any(),
// any())).thenReturn(1.0);
