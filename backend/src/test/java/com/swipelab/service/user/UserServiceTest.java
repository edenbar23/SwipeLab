...
// when(authentication.getPrincipal()).thenReturn(testUser); // Principal is the
...
// @Test
// void
// getCurrentUserProfile_AuthenticatedUserNotFoundInRepo_WhenPrincipalIsString()
// {
// mockedSecurityContextHolder.when(SecurityContextHolder::getContext).thenReturn(securityContext);
// when(securityContext.getAuthentication()).thenReturn(authentication);
// when(authentication.isAuthenticated()).thenReturn(true);
// when(authentication.getPrincipal()).thenReturn("testuser"); // Principal is
// String
