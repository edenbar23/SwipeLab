...
// any(TokenType.class), any(Duration.class)))
// .thenReturn("access-token");

// String token = jwtService.generateAccessToken(testUser);

// assertEquals("access-token", token);
// verify(tokenProvider).generateToken(eq("testuser"), eq("USER"),
// eq(TokenType.ACCESS), any(Duration.class));
// }
...
// when(tokenProvider.generateToken(anyString(), anyString(),
// eq(TokenType.REFRESH), any(Duration.class)))
...
// verify(userRepository, times(1)).save(testUser); // Saved once during
// generateRefreshToken call inside
// // refreshTokens
// }
